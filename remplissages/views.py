...
from remplissages.models import Evangelisation, Person, Site, Suivie, Profile
...
def index_rempl(request, passe=None, pk=None):
    ...
    if passe and pk:
        ...
        context = {'evang': evang_modal, 'personnes_evang_oui': personnes_evang_modal, 'passe':passe}
...
@login_required
def index_rempl_serach(request):
    evang = None
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if cd['site_evangelisation'] != evangelisation.site:
                print(evangelisation.site)
                print(cd['site_evangelisation'])
                form.add_error('site_evangelisation', msg1)
            else:
                personne = form.save(commit=False)
...
def add_boos_other(request):
    ...
    return render(request, 'remplissages/evang/add_boss_other.html', context)
